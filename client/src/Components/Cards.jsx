...
function Cards() {
  ...
  useEffect(() => {
      const fetchListings = async () => {
          try {
              const response = await axios.get('https://movieflix-gmqp.onrender.com/listings'); 
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://movieflix-gmqp.onrender.com/listings/${id}`);
  ...
  return (
    <div className="row row-cols-lg-4 row-cols-md-2 row-cols-lg-1" id="outer-div">
